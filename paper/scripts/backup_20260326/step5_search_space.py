...
Figures:
  fig5_search_space — 1×3 (clusters, search effort, ordering complexity)
...
def figure5_search_space():
    ...
    ax.plot(n_vals, [2*n for n in n_vals], 's--', color='gray', alpha=0.5,
            label='2N (no clustering)', markersize=4)
    ...
    # (b) Search effort
    ax = axes[1]
    ax.plot(n_vals, [PREFIXES_EXPLORED[n] for n in n_vals],
            marker='o', color=COLORS['OptLoad'], label='Prefixes Explored', linestyle='-')
    ax.plot(n_vals, [BACKTRACK_CALLS[n] for n in n_vals],
            marker='s', color=COLORS['OptLoad'], label='Backtrack Calls', linestyle='--')
    ax.set_xlabel('Number of Requests (N)')
    ax.set_ylabel('Count')
    ...
    ax = axes[2]
    ax.plot(n_vals, [CLUSTER_ORDERINGS[n] for n in n_vals],
            marker='o', color=COLORS['OptLoad'], label='Cluster Orderings')
    ax.plot(n_vals, [CROSS_PRODUCT[n] for n in n_vals],
            marker='s', color=COLORS['Insertion'], label='Cross Product')
    ax.plot(n_vals, [VALID_ORDERINGS[n] for n in n_vals],
            marker='^', color=COLORS['Exact'], label='Valid Orderings')
    ax.set_xlabel('Number of Requests (N)')
    ax.set_ylabel('Count')
    ...
    ax2_twin = ax2.twinx()
    ax2.bar(n_vals, [PRUNE_PCT[n] for n in n_vals], width=2.5, alpha=0.6,
            color=COLORS['OptLoad'], label='Pruning Rate (%)')
    ax2_twin.plot(n_vals, [SEED_LB_RATIO[n] for n in n_vals],
                  'D-', color=COLORS['Insertion'], label='Seed LU / LB LU')
